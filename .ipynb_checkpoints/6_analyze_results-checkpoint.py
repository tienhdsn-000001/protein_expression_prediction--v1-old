...
- FIX: Corrected the x-axis labels on the individual performance scaling
  plots. Replaced the malfunctioning PercentFormatter with a manual string
  formatting approach to correctly display the data fractions as percentages
  (e.g., "20%", "40%").
...
def plot_individual_model_performance_scaling(results_df: pd.DataFrame):
    ...
    for _, row in unique_models.iterrows():
        ...
        # --- FIX: Manually format x-axis labels to be correct percentages ---
        new_labels = [f'{int(label.get_text())}%' for label in ax.get_xticklabels()]
